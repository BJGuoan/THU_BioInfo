...
### Step 0:
```
root@528065cfd326:/home/test# cd ./linux
root@528065cfd326:/home/test/linux# gunzip 1.gtf.gz
root@528065cfd326:/home/test/linux# ls
1.gtf  file  result.gtf  test_command.gtf
```

### Step 1:
```
root@528065cfd326:/home/test/linux# cat 1.gtf | head
...
root@528065cfd326:/home/test/linux# cat 1.gtf | tail
...
root@528065cfd326:/home/test/linux# cat 1.gtf | head -15
#!genome-build R64-1-1
#!genome-version R64-1-1
#!genome-date 2011-09
#!genome-build-accession :GCA_000146045.2
...
```
```
root@528065cfd326:/home/test/linux# ls -lh 1.gtf
-rw-rw-r-- 1 test test 12M Sep 11  2018 1.gtf

root@528065cfd326:/home/test/linux# wc -l 1.gtf
42252 1.gtf
```
```
root@528065cfd326:/home/test/linux# grep -v "^#" 1.gtf | grep -v '^$' | wc -l
42247

root@528065cfd326:/home/test/linux# cat 1.gtf | awk '$0!~/^\s*$/{print}' | head -10
#!genome-build R64-1-1
#!genome-version R64-1-1
#!genome-date 2011-09
#!genome-build-accession :GCA_000146045.2
...
root@528065cfd326:/home/test/linux# grep -v '^\s*$' 1.gtf | head -10
#!genome-build R64-1-1
#!genome-version R64-1-1
#!genome-date 2011-09
#!genome-build-accession :GCA_000146045.2
...
#### Step 2.1:
```
root@528065cfd326:/home/test/linux# cat 1.gtf | awk ' { print $1, $2, $3 } ' | head
#!genome-build R64-1-1
#!genome-version R64-1-1
#!genome-date 2011-09
#!genome-build-accession :GCA_000146045.2
...
root@528065cfd326:/home/test/linux# cat 1.gtf | cut -f 1,2,3 | head
#!genome-build R64-1-1
#!genome-version R64-1-1
#!genome-date 2011-09
#!genome-build-accession :GCA_000146045.2
...
root@528065cfd326:/home/test/linux# cut -f 1,3,4,5 1.gtf | head
#!genome-build R64-1-1
#!genome-version R64-1-1
#!genome-date 2011-09
#!genome-build-accession :GCA_000146045.2
...
#### Step 2.2:
```
root@528065cfd326:/home/test/linux# cat 1.gtf | awk '$3 =="gene" { print $1, $3, $9 } ' | head
IV gene gene_id
IV gene gene_id
IV gene gene_id
IV gene gene_id
...
#### Step 3.1:
```
root@528065cfd326:/home/test/linux# grep -v '^#' 1.gtf |awk '{print $3}'| sort | uniq -c
   7050 CDS
   7553 exon
   7126 gene
   6700 start_codon
...
#### Step 3.2:
```
root@528065cfd326:/home/test/linux# cat 1.gtf | awk ' { print $3, $5-$4 + 1 } ' | head
 1
 1
 1
 1
...
```
```
root@528065cfd326:/home/test/linux# cat 1.gtf | awk 'BEGIN{size=0;}$3 =="CDS"{ len=$5-$4 + 1; size += len; print "Size:", size } ' | tail -n 1
Size: 9030648

root@528065cfd326:/home/test/linux# cat 1.gtf | awk 'BEGIN{L=0;}$3 =="CDS"{L+=$5-$4 + 1;}END{print L;}'
9030648

root@528065cfd326:/home/test/linux# cat 1.gtf | awk '$3 =="CDS"{L+=$5-$4 + 1;}END{print L;}'
9030648
```
```
root@528065cfd326:/home/test/linux# awk 'BEGIN  {s = 0;line = 0;}$3 =="CDS" && $1 =="I"{ s += $5-$4+1;line += 1}END {print "mean="s/line}' 1.gtf
mean=1239.52
```

#### Step 3.3:
```
root@528065cfd326:/home/test/linux# cat 1.gtf | awk '$3 == "gene"{split($10,x,";");name = x[1];gsub("\"", "", name);print name,$5-$4+1}' | head
YDL248W 1152
YDL247W-A 75
YDL247W 1830
YDL246C 1074
...
#### Step 4.1:
```
root@528065cfd326:/home/test/linux# grep exon 1.gtf | awk '{print $5-$4+1}' | sort -n | tail -3 > 1.txt

root@528065cfd326:/home/test/linux# cat 1.txt
12279
14730
14733
```
#### Step 4.2:
```
root@528065cfd326:/home/test/linux# vi run.sh

root@528065cfd326:/home/test/linux# cat run.sh
#!/bin/bash
grep exon *.gtf | awk '{print $5-$4+1}' | sort -n | tail -3

root@528065cfd326:/home/test/linux# chmod u+x run.sh

root@528065cfd326:/home/test/linux# ./run.sh
12279
14730
14733
```