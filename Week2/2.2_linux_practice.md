# 2.2 Linux Practice  李皓宇
#### 1.列出1.gtf文件中 XI 号染色体上的后 10 个 CDS （按照每个CDS终止位置的基因组坐标进行sort）。
```
root@2c303c39f353:/home/test/linux# awk -F'\t' '$1=="XI" && $3=="CDS" {print $0}' 1.gtf | sort -n -k 5 | tail -10
...
```
root@2c303c39f353:/home/test/linux# grep -v '^#' 1.gtf |awk '$1=="IV" {print $3}'| sort | uniq -c | sort
    853 start_codon
    853 stop_codon
    886 gene
    886 transcript
...
```
root@2c303c39f353:/home/test/linux# awk -F'\t' '$1!="IV" && $7=="-" && $3=="CDS" {print $5-$4+1}' 1.gtf | sort -n | tail -2
12276
14730
```
...
```
root@2c303c39f353:/home/test/linux# awk '$1=="XV" && $3=="gene"{split($10,x,";");name = x[1];gsub("\"", "", name);print name,$5-$4+1}' 1.gtf | sort -n -k 2 | tail -5
YOR142W-B 5269
YOR192C-B 5314
YOR343W-B 5314
YOR396W 5391
...
#### 5.统计1.gtf列数。
```
root@2c303c39f353:/home/test/linux# grep -v '^#' 1.gtf | awk -F'\t' '{print NF}' | uniq
9
```
